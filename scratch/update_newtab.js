...
content = content.replace(
  ...
  'const tagsStr = autoTags.concat(manTags).join(" ") + (bookmark.isTopSite ? " " + t(api, "frequentTag").toLowerCase() : "");'
);
...
content = content.replace(
  ...
  'if (bookmark.isTopSite) {\n        const topSiteTag = el("span", { class: "bookmark-tag manual-tag" }, t(api, "frequentTag"));\n        tagsWrapper.append(topSiteTag);\n      }\n      metaContainer.append(tagsWrapper);\n    }'
);

// 9. Hide site from topsites (Blacklist logic)
content = content.replace(
  'const folderLinksBtn = el("button", { class: "icon-button setup-nav-button", "aria-label": "Links de carpeta", title: "Links de carpeta" }, svgFolderLinks);',
  'const hideTopSiteBtn = isTopSitesFolder ? el("button", { class: "icon-button setup-nav-button", "aria-label": t(api, "hideSite"), title: t(api, "hideSite") }, svgDelete) : null;\n\n    const folderLinksBtn = el("button", { class: "icon-button setup-nav-button", "aria-label": "Links de carpeta", title: "Links de carpeta" }, svgFolderLinks);'
);

content = content.replace(
  'const folderButtons = el("div", { class: "folder-actions" }, [\n      viewButton,\n      sortBtn,\n      modeBtn',
  'const folderButtons = el("div", { class: "folder-actions" }, [\n      viewButton,\n      sortBtn,\n      modeBtn'
);

// We won't put hideTopSiteBtn at the folder level. Wait! TopSites are individual sites. The user hides an individual bookmark, not the folder!
// Let's revert the hideTopSite folder button logic.
`

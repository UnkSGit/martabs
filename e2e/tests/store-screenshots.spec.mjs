...
const screenshotsDir = path.resolve('docs/assets/store_screenshots');
...
function createMockData(theme = 'dark', language = 'es') {
  const folders = [
...
test.describe('Capturas de la tienda', () => {
  test('genera capturas en multiples idiomas', async () => {
    const server = await startStaticServer();
    const languages = ['es', 'zh_CN', 'ja', 'en'];

    for (const lang of languages) {
      const browser = await chromium.launch({
        executablePath: chromePath,
        headless: true,
        args: ['--no-sandbox', '--lang=' + lang]
      });
      ...
      try {
        ...
        async function capture(name) {
          await page.screenshot({ path: path.join(langDir, name), fullPage: false });
        }

        await installMockApi(page, createMockData('dark', lang));

        await page.goto(`${server.origin}/src/newtab/newtab.html`);
        await expect(page.locator('.group')).toHaveCount(4);
        await capture('1-dashboard-dark.png');
        ...
        await page.reload();
        await expect(page.locator('.group')).toHaveCount(4);
        await capture('2-dashboard-light.png');

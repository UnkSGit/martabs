...
test.describe('Capturas de documentacion', () => {
  test('genera capturas publicas con datos de ejemplo', async () => {
    ...
    try {
      ...
      await screenshot(page, 'dashboard-ar.png');
      ...
      await page.reload();
      await expect(page.locator('.group')).toHaveCount(4);
      // We don't necessarily need dashboard-light for arabic if they didn't ask, but let's just do dashboard-light-ar.png
      await screenshot(page, 'dashboard-light-ar.png');
      ...
      await screenshot(page, 'folder-modes-ar.png');

      await page.locator('#search').fill('docs');
      await expect(page.locator('.result')).toHaveCount(2);
      await screenshot(page, 'search-results-ar.png');
      ...
      await screenshot(page, 'edit-bookmark-ar.png');
      ...
      await page.evaluate(async () => {
        ...
        await chrome.storage.local.set({ clickStats: [
          { bookmarkId: 'b-github', url: 'https://github.com', title: 'GitHub - Proyecto martabs' },
          { bookmarkId: 'b-github', url: 'https://github.com', title: 'GitHub - Proyecto martabs' }
        ]});
      });
      await page.reload();
      ...
      await screenshot(page, 'settings-ar.png');
    } finally {
      await browser.close();
      await server.close();
    }
